...
class GenerateQuestionAPI:
    def process(task, data):
        ...
        if response.status_code != 200:
         logger.error(f"Error fetching last GenerateQuestion data. API response: {response.text}")
         raise Exception(f"Error fetching last GenerateQuestion data. API response: {response.text}")
